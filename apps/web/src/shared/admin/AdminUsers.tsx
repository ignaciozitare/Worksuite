        ...
        {done?<div className="mbody"><div className="ok-fl">✓ {t("admin.passwordChanged")}</div></div>:(
          <><div className="mbody">
            <div style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px",background:"var(--sf2)",borderRadius:"var(--r)",border:"1px solid var(--bd)"}}>
              <div className="avatar" style={{width:30,height:30,fontSize: 'var(--fs-2xs)',flexShrink:0}}>{user.avatar}</div>
              <div><div style={{fontWeight:600}}>{user.name}</div><div style={{fontFamily:"var(--mono)",fontSize: 'var(--fs-2xs)',color:"var(--tx3)"}}>{user.email}</div></div>
            </div>
    ...
    <div className="ov" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="mb" style={{maxWidth:580}}>
        ...
        {done?<div className="mbody"><div className="ok-fl">✓ {cnt} {t("admin.csvImportDone")}</div></div>:(
          <><div className="mbody">
            {!parsed&&(<><div className={`dropzone ${drag?"over":""}`} onDragOver={e=>{e.preventDefault();setDrag(true);}} onDragLeave={()=>setDrag(false)} onDrop={e=>{e.preventDefault();setDrag(false);process(e.dataTransfer.files[0]);}} onClick={()=>fileRef.current?.click()}><div style={{fontSize: 'var(--fs-xl)',marginBottom:8}}>📂</div><div style={{fontSize: 'var(--fs-xs)',color:"var(--tx2)",fontWeight:500,marginBottom:4}}>{t("admin.csvDropzone")}</div><div style={{fontSize: 'var(--fs-2xs)',color:"var(--tx3)"}}>CSV only</div><input ref={fileRef} type="file" accept=".csv" style={{display:"none"}} onChange={e=>process(e.target.files[0])}/></div><button className="btn-g" style={{alignSelf:"flex-start"}} onClick={downloadTpl}>↓ {t("admin.csvDownloadTemplate")}</button></>)}
            {parsed&&(<><div style={{display:"flex",gap:12,flexWrap:"wrap",alignItems:"center"}}><span style={{fontSize: 'var(--fs-2xs)',color:"var(--tx2)"}}><strong>{parsed.rows.length}</strong> {t("admin.csvRows")}</span><span style={{fontSize: 'var(--fs-2xs)',color:parsed.errorCount>0?"var(--amber)":"var(--tx2)"}}><strong>{parsed.errorCount}</strong> {t("admin.csvErrors")}</span><span style={{fontSize: 'var(--fs-2xs)',color:"var(--green)"}}><strong>{validRows.length}</strong> ready</span><button className="btn-g" style={{marginLeft:"auto",fontSize: 'var(--fs-2xs)'}} onClick={()=>setParsed(null)}>↩ Change</button></div><div className="csv-preview"><div className="csv-row hdr"><div className="csv-cell">#</div><div className="csv-cell">Name</div><div className="csv-cell">Email</div><div className="csv-cell">Role</div><div className="csv-cell">Status</div></div>{parsed.rows.map(r=>(<div key={r.idx} className={`csv-row ${!r.valid?"err-row":""}`}><div className="csv-cell" style={{color:"var(--tx3)"}}>{r.idx}</div><div className="csv-cell">{r.name||"—"}</div><div className="csv-cell">{r.email||"—"}</div><div className="csv-cell"><span className="r-tag r-user">{r.role}</span></div><div className="csv-err-tag">{r.valid?<span style={{color:"var(--green)"}}>✓ OK</span>:r.errors.join(" · ")}</div></div>))}</div></>)}
    ...
    <div>
      ...
      <div className="a-card" style={{padding:0,overflow:"hidden"}}>
        <table className="ut">
          ...
          <tbody>{users.map(u=>(
            <tr key={u.id}>
              <td><div style={{display:"flex",alignItems:"center",gap:8}}><button type="button" onClick={()=>setPickerUser(u)} title={t("profile.avatarChange")} style={{background:"transparent",border:"none",padding:0,cursor:"pointer",borderRadius:"50%"}}><UserAvatar user={{id:u.id,name:u.name,email:u.email,avatar:u.avatar,avatarUrl:u.avatar_url ?? u.avatarUrl ?? null}} size={26} imageWidth={64}/></button><span style={{fontWeight:500}}>{u.name}</span>{u.id===currentUser.id&&<span style={{fontSize: 'var(--fs-2xs)',color:"var(--tx3)"}}>{t("admin.you")}</span>}</div></td>
              <td style={{fontFamily:"var(--mono)",fontSize: 'var(--fs-2xs)',color:"var(--tx3)"}}>{u.email}</td>
              <td><span className={`r-tag ${u.role==="admin"?"r-admin":"r-user"}`}>{u.role==="admin"?t("admin.roleAdmin"):t("admin.roleUser")}</span></td>
              <td><div style={{display:"flex",gap:3}}>{[DeskType.NONE, DeskType.HOTDESK, DeskType.FIXED].map(dt=>(<button key={dt} onClick={()=>changeDeskType(u.id,dt)} style={{fontSize: 'var(--fs-2xs)',fontWeight:700,padding:"2px 6px",borderRadius:3,border:`1px solid ${u.deskType===dt?DESK_COLORS[dt]:"var(--bd)"}`,background:u.deskType===dt?`${DESK_COLORS[dt]}15`:"transparent",color:u.deskType===dt?DESK_COLORS[dt]:"var(--tx3)",cursor:"pointer"}}>{DESK_LABELS[dt]}</button>))}</div></td>
              <td><div style={{display:"flex",gap:3,flexWrap:"wrap"}}>{MODULES.map(m=>{const hasMod=(u.modules||["jt","hd","retro","deploy"]).includes(m.id);return(<button key={m.id} onClick={()=>toggleModule(u.id,m.id)} style={{fontSize: 'var(--fs-2xs)',fontWeight:700,padding:"2px 7px",borderRadius:3,border:`1px solid ${hasMod?m.color:"var(--bd)"}`,background:hasMod?`${m.color}18`:"transparent",color:hasMod?m.color:"var(--tx3)",cursor:"pointer",textDecoration:hasMod?"none":"line-through"}}>{m.id.toUpperCase()}</button>);})}</div></td>
              <td><span style={{fontSize: 'var(--fs-2xs)',fontWeight:500,color:u.active?"var(--green)":"var(--red)"}}>{u.active?t("admin.statusActive"):t("admin.statusBlocked")}</span></td>
              <td>
