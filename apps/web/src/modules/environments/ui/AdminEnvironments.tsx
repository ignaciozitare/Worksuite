...
// ── Dual-panel drag & drop picker ────────────────────────────────────────────
function DualPanelPicker({ label, allItems, selected, onAdd, onRemove }) {
  ...
  const selectedItems = selected.map(v => allItems.find(it => it.value === v)).filter(Boolean);
  ...
  const listStyle = { flex: 1, overflowY: 'auto', padding: 4 };
  const itemBase = {
          ...
          <div style={listStyle}>
            {filteredSel.map(it => (
              <div key={it.value} draggable
                ...
                onMouseLeave={e => (e.currentTarget.style.background = 'rgba(79,110,247,.08)')}>
                <span style={{ fontSize: 10, color: 'var(--tx3,#50506a)' }}>⠿</span>
                <span style={{ flex: 1 }}>{it.label}</span>
                {it.hint && <span style={{ fontSize: 9, color: 'var(--tx3,#50506a)' }}>{it.hint}</span>}
