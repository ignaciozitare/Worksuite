  ...
  return (
    <div>
      ...
      <div style={{marginBottom:24}}>
        ...
        <p style={{fontSize: 'var(--fs-xs)',color:'var(--tx3)',marginTop:4,maxWidth:600}}>
        ...
        {selected.size > 0 && (
          <div style={{display:'flex',alignItems:'center',gap:8,marginLeft:'auto'}}>
            <select value={assignWfId} onChange={e => setAssignWfId(e.target.value)}
              style={{padding:'6px 10px',fontSize: 'var(--fs-xs)',background:'var(--sf2)',border:'1px solid var(--bd)',
              ...
              <tr key={tt.id} style={{transition:'background .12s'}}
                ...
                <td style={tdStyle}>
                  <div style={{display:'flex',alignItems:'center',gap:8}}>
                    <span className="material-symbols-outlined" style={{fontSize: 'var(--icon-md)',color:'var(--ac)'}}>{tt.icon || 'task_alt'}</span>
                    <div>
                      <div style={{fontSize: 'var(--fs-xs)',fontWeight:600,color:'var(--tx)'}}>{tt.name}</div>
                      <div style={{fontSize: 'var(--fs-2xs)',color:'var(--tx3)'}}>{(tt.schema as unknown[]).length} {t('vectorLogic.states').toLowerCase()}</div>
                    </div>
                  </div>
                </td>
                <td style={tdStyle}>
                  <span style={{
                    padding:'3px 10px',borderRadius:20,fontSize: 'var(--fs-2xs)',fontWeight:600,
                ...
                <td style={tdStyle}>
                  <button onClick={() => taskTypeRepo.remove(tt.id).then(() => setTaskTypes(p => p.filter(x => x.id !== tt.id)))}
                    style={{background:'none',border:'none',cursor:'pointer',color:'var(--red)',fontSize: 'var(--fs-xs)',fontFamily:'inherit'}}>
        ...
        <div style={{position:'fixed',inset:0,zIndex:200,display:'flex',alignItems:'center',
          ...
          <div style={{background:'var(--sf)',border:'1px solid var(--bd)',borderRadius:16,
            ...
            <div style={{padding:'16px 20px',borderBottom:'1px solid var(--bd)',
              ...
              <h3 style={{fontSize: 'var(--fs-sm)',fontWeight:700,color:'var(--tx)',margin:0}}>{t('vectorLogic.taskTypes')}</h3>
...
const thStyle = {
  padding: '10px 16px', fontSize: 'var(--fs-2xs)', fontWeight: 700, color: 'var(--tx3)',
  textTransform: 'uppercase', letterSpacing: '.08em', textAlign: 'left',
  background: 'var(--sf3)', borderBottom: '1px solid var(--bd)',
};

const tdStyle = {
  padding: '12px 16px', fontSize: 'var(--fs-xs)', color: 'var(--tx)',
