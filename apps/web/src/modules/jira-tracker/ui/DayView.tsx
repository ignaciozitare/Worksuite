...
export function DayView({ date, filters, worklogs, onDateChange, onOpenLog, onDeleteWorklog }: DayViewProps) {
  ...
  return (
    <div>
      ...
      {fl.length===0&&<div className="empty"><div className="empty-i">📭</div><div>{t("jiraTracker.noWorklogs")}</div><div style={{fontSize:'var(--fs-2xs)'}}>{t("jiraTracker.noWorklogsSub")}</div><button className="btn-log" style={{marginTop:10}} onClick={()=>onOpenLog({date})}>{t("jiraTracker.logThisDay")}</button></div>}
      {eps.map(ep=>{
