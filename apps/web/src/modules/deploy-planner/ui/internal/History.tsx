...
type SortBy = 'end_date' | 'start_date' | 'bugs';
...
export function History({ releases, tickets, setDetail, statusCfg, classifiedSubs = [] }: HistoryProps) {
  const [sortBy, setSortBy] = useState<SortBy>('end_date');
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        <h2 style={{ fontSize: 'var(--fs-sm)', color: 'var(--dp-tx)', fontWeight: 700 }}>Historial</h2>
        <span style={{ fontSize: 'var(--fs-2xs)', color: 'var(--dp-tx3)' }}>{sorted.length} releases</span>
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
          <SLabel>Ordenar</SLabel>
          <select
            value={sortBy}
            onChange={e => setSortBy(e.target.value as SortBy)}
            style={{ background: 'var(--dp-sf)', border: '1px solid var(--dp-bd)', borderRadius: 4, padding: '4px 8px', fontSize: 'var(--fs-2xs)', color: 'var(--dp-tx2)', outline: 'none' }}
          >
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 'var(--fs-2xs)' }}>
        <thead><tr style={{ borderBottom: '1px solid var(--dp-bd)' }}>
          {['RELEASE', 'ESTADO', 'INICIO', 'FIN', 'DURACIÓN', 'TICKETS', 'BUGS', 'REPOS'].map(h => (
            <th key={h} style={{ padding: '7px 12px', textAlign: 'left', color: 'var(--dp-tx3)', fontWeight: 600, letterSpacing: '.06em', fontSize: 'var(--fs-2xs)' }}>{h}</th>
          ))}
        </tr></thead>
        <tbody>
          {sorted.map(rel => {
            ...
            return (
              <tr
                ...
                style={{ borderBottom: '1px solid var(--dp-bd)', cursor: 'pointer' }}
                onMouseEnter={e => (e.currentTarget.style.background = 'var(--dp-sf)')}
                onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
              >
                <td style={{ padding: '11px 12px' }}>
                  <div style={{ fontWeight: 700, color: 'var(--dp-primary)', fontSize: 'var(--fs-2xs)' }}>{rel.release_number || '—'}</div>
                  {rel.description && <div style={{ color: 'var(--dp-tx3)', fontSize: 'var(--fs-2xs)', marginTop: 1 }}>{rel.description}</div>}
                </td>
                <td style={{ padding: '11px 12px' }}>
                  <span style={{ padding: '2px 8px', borderRadius: 4, fontSize: 'var(--fs-2xs)', fontWeight: 700, color: cfg.color, background: cfg.bg_color, border: `1px solid ${cfg.border}` }}>
                    {rel.status || 'Planned'}
                  </span>
                </td>
                <td style={{ padding: '11px 12px', color: 'var(--dp-tx2)' }}>{rel.start_date ?? '—'}</td>
                <td style={{ padding: '11px 12px', color: 'var(--dp-tx2)' }}>{rel.end_date ?? '—'}</td>
                <td style={{ padding: '11px 12px', color: 'var(--dp-tx)', fontWeight: 600 }}>{dur != null ? `${dur}d` : '—'}</td>
                <td style={{ padding: '11px 12px', color: 'var(--dp-tx2)' }}>{rel.ticket_ids?.length ?? 0}</td>
                <td style={{ padding: '11px 12px' }}>{(() => {
                ...
                <td style={{ padding: '11px 12px' }}>
                  <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                    ...
                    {relRepos.length > 3 && <span style={{ fontSize: 'var(--fs-2xs)', color: 'var(--dp-tx3)' }}>+{relRepos.length - 3}</span>}
                  </div>
                </td>
              </tr>
            );
