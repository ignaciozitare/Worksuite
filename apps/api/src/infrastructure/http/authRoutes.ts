...
export async function authRoutes(app: FastifyInstance, opts: AuthPluginOptions): Promise<void> {
  ...
  app.post('/login', async (request, reply) => {
    ...
    if (error || !data.user) return reply.status(401).send({ ok: false, error: { code: 'INVALID_CREDENTIALS', message: 'Invalid email or password' } });
    ...
    const token = app.jwt.sign({ sub: data.user.id, email: data.user.email, role: profile.role, name: profile.name, avatar: profile.avatar }, { expiresIn: '7d' });
    return reply.send({ ok: true, data: { token, user: profile } });
  });

  app.get('/me', { preHandler: [app.authenticate] }, async (request, reply) => {
    const payload = request.user as { sub: string };
    const { data: profile, error } = await supabase.from('users').select('*').eq('id', payload.sub).single();
    if (error || !profile) return reply.status(404).send({ ok: false, error: { code: 'USER_NOT_FOUND', message: 'User not found' } });
