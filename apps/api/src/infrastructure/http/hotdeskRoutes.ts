...
export async function hotdeskRoutes(app: FastifyInstance, opts: HotDeskPluginOptions): Promise<void> {
  ...
  app.get('/map', auth, async (request, reply) => {
    ...
    const [seats, reservations, fixedAssignments] = await Promise.all([hotdeskRepo.getSeats(), hotdeskRepo.getReservations(date, date), hotdeskRepo.getFixedAssignments()]);
  ...
  app.get('/table', auth, async (request, reply) => {
    ...
    const [seats, reservations, fixedAssignments] = await Promise.all([hotdeskRepo.getSeats(), hotdeskRepo.getReservations(from, to), hotdeskRepo.getFixedAssignments()]);
  ...
  app.post('/reservations', auth, async (request, reply) => {
    ...
    const output = await useCase.execute({ seatId: result.data.seatId, dates: result.data.dates, userId: user.sub, userName: user.name });
  ...
  app.delete('/reservations/:seatId/:date', auth, async (request, reply) => {
    ...
    try {
      ...
      return reply.status(status).send({ ok: false, error: { code: 'DOMAIN_ERROR', message: msg } });
    }
  });
}
