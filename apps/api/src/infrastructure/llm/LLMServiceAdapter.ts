// @ts-nocheck
// ─────────────────────────────────────────────────────────────────────────────
// LLM Service adapter — implements ILLMService for Anthropic, OpenAI, and
// Google Gemini. Runs server-side so API keys are never exposed to the browser.
// ─────────────────────────────────────────────────────────────────────────────
...
export class LLMServiceAdapter implements ILLMService {
  async chat(req: LLMChatRequest): Promise<LLMResponse> {
    ...
    if (req.provider === 'google') return this.callGoogle(req);
  ...
  async listModels(provider: LLMProvider, apiKey: string): Promise<LLMModel[]> {
    ...
    if (provider === 'google') return this.listGoogleModels(apiKey);
  ...
  private async callGoogle(req: LLMChatRequest): Promise<LLMResponse> {
    // Gemini uses "contents" with role "user" or "model" instead of
    // "messages" / "assistant". System instructions go in a separate field.
  ...
  private async listGoogleModels(apiKey: string): Promise<LLMModel[]> {
    const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${encodeURIComponent(apiKey)}`);
    if (!res.ok) {
      const err = await res.text();
      throw new Error(`Google Gemini models error ${res.status}: ${err}`);
    }
    const data = await res.json() as {
      models?: Array<{
        name: string;
    ...
    return (data.models ?? [])
      ...
      // Strip the "models/" prefix that the list endpoint includes
      .map((m) => ({
