// ─────────────────────────────────────────────────────────────────────────────
// DOMAIN — HotDesk
// Business rules: reservations, conflicts, fixed seats.
// ─────────────────────────────────────────────────────────────────────────────
...
export class Seat {
  constructor(
    ...
    readonly coordinates: SeatCoordinates,
  ) {
...
export class SeatReservation {
  ...
  static create(seatId: string, userId: string, userName: string, date: string): SeatReservation {
    ...
    return new SeatReservation(
      `res-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
      seatId,
      userId,
      userName,
      date,
  ...
  static reconstitute(props: {
    id: string; seatId: string; userId: string;
    userName: string; date: string; createdAt: string;
  }): SeatReservation {
    return new SeatReservation(
      props.id, props.seatId, props.userId,
      props.userName, props.date, props.createdAt,
    );
  }
}
...
// ── Domain Service — all reservation rules live here ──────────────────────────
export class ReservationService {
  ...
  static canReserve(
    ...
    if (this.isWeekend(date)) {
      return { allowed: false, reason: 'Cannot reserve on weekends' };
    }
    if (this.isFixed(seatId, fixedAssignments)) {
      return { allowed: false, reason: 'Seat is permanently assigned' };
    }
    const existing = this.findReservation(seatId, date, reservations);
    if (existing && existing.userId !== userId) {
      return { allowed: false, reason: 'Seat already reserved by another user' };
    }
    return { allowed: true };
  }
  ...
  static canRelease(
    ...
    if (!reservation) {
      return { allowed: false, reason: 'No reservation found' };
    }
    if (reservation.userId !== userId) {
      return { allowed: false, reason: 'Can only release your own reservation' };
    }
    return { allowed: true };
  }
}
