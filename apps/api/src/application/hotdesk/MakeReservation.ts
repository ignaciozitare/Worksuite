...
export interface MakeReservationInput {
  seatId: string;
  dates: string[];   // YYYY-MM-DD — supports multi-day booking
  userId: string;
  userName: string;
}
...
export class MakeReservation {
  ...
  async execute(input: MakeReservationInput): Promise<MakeReservationOutput> {
    ...
    for (const date of input.dates) {
      const check = ReservationService.canReserve(
        input.seatId, date, input.userId, fixedAssignments, allReservations,
      );

      if (!check.allowed) {
        skipped.push({ date, reason: check.reason! });
        continue;
      }

      const reservation = SeatReservation.create(
        input.seatId, input.userId, input.userName, date,
      );
